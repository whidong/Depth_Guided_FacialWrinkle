...
def main_experiment(run_id, seed, mode, model_type, batch_size = 10, pretrain_path=None):
    """
    ...
    if model_type == "custom_unet":
        in_ch = {"RGB": 3, "RGBT": 4, "RGBDT": 5, "denoise": 3, "image": 3}.get(mode, None)
        if in_ch is None:
    ...
    model_pth_dir = "./checkpoint/fine_tuning"
    ...
    epochs = 150
    scaler = GradScaler()
    writer = SummaryWriter(log_dir=f'./tensorboard/finetune_{model_type}_{mode}_{run_id}_seed{seed}')
    ...
    for epoch in range(epochs):
        ...
        if val_jsi > best_val_jsi and val_loss < best_val_loss_unet:
            ...
            save_model(model, optimizer, scheduler, epoch + 1, best_val_loss_unet, f'./checkpoint/fine_tuning/best_{model_type}_{mode}_{run_id}_seed{seed}.pth')
...
def main():
    ...
    mode = "RGBT"
    batch_size = 4
    pretrain_path ="./checkpoint/pretrain/best_pretrain_unet_RGB_seed42.pth"
    results = []
    for run_id, seed in enumerate(run_seeds):
        val_loss, val_jsi = main_experiment(run_id, seed, mode, model_type, batch_size, pretrain_path)
